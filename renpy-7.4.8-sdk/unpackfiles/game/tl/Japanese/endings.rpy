# TODO: Translation updated at 2022-06-07 01:45

# game/endings.rpy:11
translate Japanese bad_ending1_9de101e6:

    # "Some time later . . . who knows when . . . I wake up on the concrete floor of a dark cell."
    "それからしばらくして……いつかは分からない……俺は暗い独房のコンクリートの床で目を覚ました。"

# game/endings.rpy:12
translate Japanese bad_ending1_d1f92089:

    # me "Oww . . . my head."
    me "うっ……頭が……"

# game/endings.rpy:13
translate Japanese bad_ending1_8fc3cdbd:

    # "Rubbing the wound on my forehead, I try to sit up."
    "額の傷をさすりながら、なんとか上体を起こそうとする。"

# game/endings.rpy:14
translate Japanese bad_ending1_94735d05:

    # gestapo determined "Finally awake, are we?"
    gestapo determined "ようやく目が覚めたか？"

# game/endings.rpy:15
translate Japanese bad_ending1_4cdeda93:

    # "Through a pair of iron bars, a cocky young officer smirks at me, brandishing a pair of leather gloves."
    "鉄格子越しに、皮手袋を見せびらかすようにした生意気そうな若い将校がニヤリと笑っている。"

# game/endings.rpy:16
translate Japanese bad_ending1_d3761044:

    # me "W-Who are you guys? What's going on?"
    me "だ、誰だお前らは！？　何が起きてるんだ！"

# game/endings.rpy:20
translate Japanese bad_ending1_57ac52a7:

    # gestapo "Us? We're the {i}Goosetapo{/i}. And we're the last people that will see you alive, Commander Yamato."
    gestapo "俺たちか？ 俺たちは{i}グースタポ{/i}。そしてお前が生きて会う最後の相手だ、ヤマト司令官。"

# game/endings.rpy:21
translate Japanese bad_ending1_3852169c:

    # "The {i}Goosetapo{/i} are Germania's secret police force, responsible for spying on, and interrogating, suspect citizens."
    "{i}グースタポ{/i}はゲルマニアの秘密警察で、市民の監視や尋問を行う組織だ。"

# game/endings.rpy:22
translate Japanese bad_ending1_4a12991d:

    # me "What is this? You have to let me go. I didn't do anything!"
    me "ふざけるな！　俺は何もしてない、帰してくれ！"

# game/endings.rpy:23
translate Japanese bad_ending1_d8763b08:

    # gestapo "You insulted the Füühbar and disobeyed orders. That's pretty treasonous behavior."
    gestapo "総統閣下を侮辱し、命令に背いた。それだけで十分に反逆罪だ。"

# game/endings.rpy:24
translate Japanese bad_ending1_00cc6bff:

    # "Just because I turned down a request, she threw me in prison to be tortured?"
    "たった一度命令を断っただけで、拷問されるために投獄されたっていうのか？"

# game/endings.rpy:25
translate Japanese bad_ending1_ff7862d1:

    # me "I apologize, I didn't mean . . . I just want to go home."
    me "悪かった……そんなつもりじゃ……帰してくれよ。"

# game/endings.rpy:26
translate Japanese bad_ending1_f691c725:

    # gestapo "Don't you get it yet? This is it for you. Nobody turns down Adorofia Hitora and lives."
    gestapo "まだ分からないのか？　お前はもう終わりだ。アドロフィア・ヒトラの命令に背いた者に未来はない。"

# game/endings.rpy:27
translate Japanese bad_ending1_1479d58c:

    # me "W-What?"
    me "な、なんだって……？"

# game/endings.rpy:31
translate Japanese bad_ending1_5b43e46b:

    # gestapo "You'll be shot at dawn. That's the end of the matter. Bet you feel stupid now."
    gestapo "夜明けとともに銃殺だ。それで終わりさ。今頃後悔してるんじゃないか？"

# game/endings.rpy:35
translate Japanese bad_ending1_ef99511f:

    # "The young {i}Goosetapo{/i} officer turns around and begins to walk away."
    "若い{i}グースタポ{/i}の将校は背を向け、静かに歩き去っていく。"

# game/endings.rpy:36
translate Japanese bad_ending1_141323aa:

    # gestapo "Oh, and it's going to be liver and sausage for your final dinner."
    gestapo "そうだ、お前の最後の晩餐はレバーとソーセージだ。"

# game/endings.rpy:40
translate Japanese bad_ending1_dc6f844a:

    # "With a loud slam, he goes through a heavy metal door and I'm left in silence."
    "金属製の重たい扉がガチャンと音を立てて閉まり、独房には沈黙だけが残る。"

# game/endings.rpy:41
translate Japanese bad_ending1_3df35ca5:

    # me "Well . . . that's the end of me . . ."
    me "……もう終わりだ……"

# game/endings.rpy:66
translate Japanese bad_ending2_f731e312:

    # "But as time passes, I find I can't keep up the farce."
    "だが、時が経つにつれ、俺はこの茶番を続けることができなくなってきた。"

# game/endings.rpy:67
translate Japanese bad_ending2_20c5d369:

    # "The inner circle, Altberlin, the tactics, the meetings, the politics, the lying and the deceit . . ."
    "内輪の連中、アルトベルリン、戦術、会議、政治、嘘と欺瞞……"

# game/endings.rpy:68
translate Japanese bad_ending2_0731a8e6:

    # "We fight battle after battle, bombing cities full of civilians and destroying peaceful nations."
    "戦いに次ぐ戦いで、民間人の住む都市を爆撃し、平和な国々を滅ぼしていく。"

# game/endings.rpy:69
translate Japanese bad_ending2_cb4ba8c9:

    # "Tanks, planes and soldiers flood across borders, incinerating towns, deporting dissidents, stealing resources . . ."
    "戦車、戦闘機、兵士が国境を越えてなだれ込み、町を焼き払い、反体制派を追放し、資源を奪い取る……"

# game/endings.rpy:70
translate Japanese bad_ending2_56bc6aab:

    # "The Füühbar's lust for power knows no bounds."
    "総統の権力欲はとどまることを知らない"

# game/endings.rpy:71
translate Japanese bad_ending2_05effd1a:

    # me "I can't go on like this. If I stay here, I'll be forced to chew on a cyanide capsule before the end . . ."
    me "こんなのもう無理だ。このままじゃ、最期には青酸カプセルを噛まされる羽目になる……"

# game/endings.rpy:72
translate Japanese bad_ending2_a1227f19:

    # "This is the Germanian empire I started to fight for . . . the one for which I was summoned to spy on."
    "これが俺が戦うことになったゲルマニア帝国……スパイとして召喚された理由だ。"

# game/endings.rpy:73
translate Japanese bad_ending2_2dfbdfbd:

    # "I didn't realize the extent of its evil. And now I've had a hand in it. But no longer."
    "その邪悪さの深さに気づいていなかった。そして今ではその一端を担ってしまった。でも、もうやめだ。"

# game/endings.rpy:74
translate Japanese bad_ending2_bd804606:

    # "Not being able to stand the shame of it . . . not being able to stomach the fight or the duplicity."
    "その恥に耐えられず……戦いや欺瞞に耐えきれなくなった。"

# game/endings.rpy:75
translate Japanese bad_ending2_cb721a84:

    # "In a cowardly manner, I resign my post and buy a one-way ticket back to Zipangu."
    "俺は臆病にも任務を放棄し、片道切符でジパングへ帰ることを決めた。"

# game/endings.rpy:76
translate Japanese bad_ending2_302ba474:

    # "Sneaking aboard a ferry bound for Edooh, I feel the weight slipping from my shoulders as we steam across the oceans."
    "江戸行きのフェリーに忍び込み、海を越えていく中で、肩の荷が少しずつ下りていくのを感じた。"

# game/endings.rpy:77
translate Japanese bad_ending2_2ee78234:

    # "Perhaps my homeland will survive this global conquest yet . . ."
    "もしかすると、俺の故郷はこの世界征服の波を生き延びるかもしれない……"

# game/endings.rpy:97
translate Japanese bad_ending3_c19cc7ee:

    # "Unable to fulfill my original duty and my mission, I leave for Zipangu."
    "本来の任務も使命も果たせず、俺はジパングへと向かった。"

# game/endings.rpy:98
translate Japanese bad_ending3_597fcad3:

    # "Whatever happens to Sovia, to Germania or to the dictators . . . who cares. It's not my responsibility any more."
    "ソヴィア、ゲルマニア、独裁者たちがどうなろうと……知ったことか。もう俺の責任じゃない。"

# game/endings.rpy:99
translate Japanese bad_ending3_cca3687c:

    # "I soon learn about the attack on Amerika and the new world war about to begin."
    "やがて、アメリカへの攻撃と新たな世界大戦の始まりを知ることになる。"

# game/endings.rpy:100
translate Japanese bad_ending3_1bd6910d:

    # "Climbing aboard a train bound for the east, I feel the weight slipping from my shoulders."
    "東へ向かう列車に乗り込みながら、肩の荷が少しずつ軽くなるのを感じていた。"

# game/endings.rpy:101
translate Japanese bad_ending3_2ee78234:

    # "Perhaps my homeland will survive this global conquest yet . . ."
    "もしかすると、俺の故郷はこの世界征服の波を生き延びるかもしれない……"

